...
export default function InternationalRates() {
  ...
  useEffect(() => {
    const loadRates = async () => {
      ...
      const res = await fetch(`/api/rates/international?version=${encodeURIComponent(selectedVersion)}`);
      const data = await res.json();
      if (res.ok) {
        setItems(data.items || []);
        setPage(1);
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">International Rates</h1>
            <p className="text-muted-foreground">Select a version to view its international rates</p>
          </div>
        ...
        <Card>
          <CardContent>
            <div className="text-lg font-semibold mb-2">International Rate Table</div>
